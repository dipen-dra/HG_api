...
export const registerUser = async (req, res) => {
    ...
    try {
        ...
        const newUser = new User({ email, fullName, password: hashedPassword });
        await newUser.save();
        return res.status(201).json({ success: true, message: "User registered successfully.", data: createUserData(newUser) });
    } catch (e) {
        console.error("Registration Error:", e);
        return res.status(500).json({ success: false, message: "Server error during registration." });
    }
...
export const loginUser = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ _id: user._id, role: user.role }, process.env.SECRET, { expiresIn: '1d' });
        res.status(200).json({ success: true, data: createUserData(user), token });
    } catch (error) {
        console.error('Login error:', error);
        res.status(500).json({ success: false, message: 'Server error during login.' });
    }
...
export const updateUserProfile = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ success: true, message: 'Profile updated successfully', data: createUserData(updatedUser) });
    } catch (error) {
        console.error("Update Profile Error:", error);
        res.status(500).json({ success: false, message: 'Server error while updating profile' });
    }
...
export const updateUserProfilePicture = async (req, res) => {
    ...
    try {
        ...
        const updatedUser = await user.save();
        res.status(200).json({ success: true, message: "Profile picture updated successfully.", data: createUserData(updatedUser) });
    } catch (error) {
        console.error("Profile picture update error:", error);
        res.status(500).json({ success: false, message: "Server error during file update." });
    }
